...
This is a repo for a simple, command line UNIX-y daemon called mqttdmx.  It's designed to connect to an MQTT server and receive lighting commands, then drive a DMX universe through a USB DMX interface (an Enttec DMX USB Pro) based on those commands.

The original Node.js prototype is stored in the /legacy folder.  You can refer to it for the feature set and the MQTT message formats the rebuild grew out of.  It is no longer maintained.

The daemon is written in Rust and shares its stack and conventions with its sister project mqttaudio (https://github.com/bandrews/mqttaudio).
...
- Run validation locally with scripts/validate.sh (Docker-based Linux lane, or --native). GitHub Actions is intentionally disabled to avoid hosted CI costs and quota usage. Do not add or enable GitHub Actions workflows, or re-enable Actions for this repository, unless the user explicitly requests it.
